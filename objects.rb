...
class Pet
  ...
  def initialize(name:)
    @name = name #@instance_var = local var
  end
...
class Dog < Pet
  ...
  def initialize(**attributes) #or you can write name:
    super
  end
...
new_pet = Pet.new(name: 'Reginald')
new_pet=(name: 'Snookums') #var=(new value) uses the setter method
